...
export const AuthForm = ({ activeTab }: { activeTab: 'signin' | 'signup' }) => {
  ...
  // --------------------------
  // DIRECT SIGNUP / SIGNIN (OTP DISABLED)
  // --------------------------
  const handleDirectAuth = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!formData.email || (!isForgotPass && !formData.password)) {
  ...
  return (
    <div className="w-full max-w-md">
      {errorMessage && (
        <div className="mb-6 px-3 py-2 bg-red-50 border border-red-200 rounded-lg">
      ...
      {!isForgotPass ? (
        <form onSubmit={handleDirectAuth} className="space-y-6">
          {activeTab === 'signup' && (
            <InputGroup
              type="text"
              name="fullname"
